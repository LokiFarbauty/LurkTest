...
async def getter_random_post(A, dialog_manager: DialogManager):
...
async def getter_random_post_start(**_kwargs):
...
dialog_random_post = Dialog(
    Window(
            ...
            getter=getter_random_post_start,
            state=states.SG_random.start,
        ),
    on_process_result=finish,
    on_start=getter_random_post,
)
